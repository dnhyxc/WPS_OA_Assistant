...
function pOpenFile(doc, params, isOnlineDoc) {
  ...
  // 更改：增加下载时有水印的判断逻辑处理（开始位置）
  var downloadParams =
    GetDocParamsValue(doc, constStrEnum.downloadParams) || {};
  if (downloadParams.watermark) {
    console.log("默认开始插入水印", downloadParams.watermark);
    DoInsertWaterToDoc(downloadParams.watermark);
    // OnBtnSaveAsLocalFile(true)
  }
  // 更改结束位置
...
// 更改：增加从OA-web端点击正文模板方法：InsertPresetTemplateDoc()
/**
 * 从OA-web端点击正文模板
 *  doc : 需要存在以下属性
...
function InsertPresetTemplateDoc(doc) {
  ...
  selection.Cut();
  selection.InsertFile(bodyTemplateUrl);

  // 恢复修订模式(根据传入参数决定)
  var l_revisionCtrl = GetDocParamsValue(activeDoc, constStrEnum.revisionCtrl);
  activeDoc.TrackRevisions = !l_revisionCtrl
    ? false
    : l_revisionCtrl.bOpenRevision;
}
...
function handleFileAndUpload(suffix, doc, uploadPath, FieldName, saveType = 1) {
  ...
  switch (suffix.toLocaleLowerCase()) {
    ...
    case ".uof":
      ...
      doc.ExportAsFixedFormat(
        l_strPath,
        (wps.Enum && wps.Enum.wdFormatOpenDocumentText) || 23,
        true
      ); //转换文件格式
...
// 更改：传入 saveType = 1 参数
function OnChangeSuffixUploadSuccess(response, saveType = 1) {
  l_result = handleResultBody(response);
  //   alert("文件转格式保存成功！");
  console.log("文件转格式保存成功！");
  OnUploadToServerSuccess(response, saveType);
}
...
function OpenTimerRun(funcCallBack) {
  ...
  // 对间隔时间做处理
  var l_AutoSaveToServerTime = wps.PluginStorage.getItem(
    constStrEnum.AutoSaveToServerTime
  );
  ...
  l_timeID = setInterval(function () {
    ...
    if (l_mCount > l_AutoSaveToServerTime) {
      //l_AutoSaveToServerTime 值由系统配置时设定，见pInitParameters()函数
      l_mCount = 0;
  ...
  Object.keys(fieldObjEnum).forEach((key) => {
    ...
    if (key === "附件") {
      ...
      currentValue.forEach((item) => {
        ...
        var currentKey = key + insertIndex;
        ...
        var bookmark = bookMarks.Item(currentKey);
        ...
        var rangeEnd = range.Start + item.name.length;

        range.SetRange(rangeStart, rangeEnd);
        bookmark.Range.Select(); // 内容替换后不会刷新，让wps作一次刷新，这可能是wps的bug

        if (!bookMarks.Item(bookMarkName)) {
          bookMarks.Add(bookMarkName, range);
        }

        var linkUrl = item.downloadUrl + "&filename=" + item.name;
...
function pInsertRInedHead(doc, strFile, bookmarkStart, bookmarkEnd) {
  ...
  if (bookMarks.Exists(bookmarkStart) && bookMarks.Exists(bookmarkEnd)) {
    ...
    console.log("当前插入位置正文位置------", insertRang);
...
// 更改：改变套红书签插入方式逻辑
function InsertRedHeadDoc(doc, callback) {
  //插入红头
  if (!doc) {
    alert("文档不存在!");
    return;

...
function OnDocumentBeforeSave(doc) {
  ...
  if (
    ...
    doc.DocID == wps.WpsApplication().ActiveDocument.DocID
  ) {
...
// 更改：增加是否有弹窗提示的判断hasCloseConfirmTip
function OnDocumentBeforeClose(doc) {
  console.log("OnDocumentBeforeClose");
  var l_fullName = doc.FullName;
  var l_bIsOADoc = false;
  ...
  // 自定义增加的内容开始
  var hasCloseConfirmTip =
    wps.PluginStorage.getItem(constStrEnum.CloseConfirmTip) || false;
  wps.PluginStorage.setItem(constStrEnum.CloseConfirmTip, true);
  // 自定义增加的内容结束
  ...
  //判断是否只读的文档，或受保护的文档，对于只读的文档，不给予保存提示
  if (pISOADocReadOnly(doc) == false) {
    if (hasCloseConfirmTip || doc.Saved == false) {
      //如果OA文档关闭前，有未保存的数据
      if (wps.confirm("是否不保存正文关闭？")) {
        wps.ApiEvent.Cancel = true;
        return;
      }
    }

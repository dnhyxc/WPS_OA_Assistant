...
function activeTab() {
  ...
  if (wps.ribbonUI) wps.ribbonUI.ActivateTab("WPSWorkExtTab");
}
...
function getDemoTemplatePath() {
  ...
  if (url.length !== 0) url = url.concat("/template/红头文件.docx");
...
function getDemoSealPath() {
  ...
  if (url.length !== 0) url = url.concat("/template/OA模板：公章.png");
